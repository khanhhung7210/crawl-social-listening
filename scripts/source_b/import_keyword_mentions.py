...
def _sentiment_for_text(
    ...
    *,
    ...
    """Prefer Source B precomputed sentiment; else shared provider."""
    ...
    return detect_mention_sentiment(text, rating=rating)
...
def upsert_mention_for_post(
    ...
    # Facebook album photo URLs are comment surfaces only — caption belongs to parent post.
    if (
        item.get("skip_post_mention")
        or is_facebook_album_photo_url(permalink)
        or (platform == "google" and is_google_maps_ui_junk(text))
    ):
        return False
    ...
    if not brands:
        return False
    sentiment, sentiment_provider = _sentiment_for_text(text, precomputed=item)
...
def upsert_comment_mentions(
    ...
    for c in comments or []:
        ...
        sentiment, sentiment_provider = _sentiment_for_text(
            text, rating=rating_num, precomputed=c
        )
        meta_obj = json.loads(comment_meta) if isinstance(comment_meta, str) else dict(comment_meta)
        sentiment_score, meta_obj = _merge_sentiment_metadata(meta_obj, c)
        mention_meta = json.dumps(meta_obj, ensure_ascii=False, default=str)
        ...
        if mrow:
            ...
            cur.execute(
                ...
                INSERT INTO mentions (
                    ...
                    'comment', %s, %s, %s, %s, %s, 'earned', %s, %s, %s, %s, %s, %s::jsonb
                )
                RETURNING mention_id::text
                """,
                (

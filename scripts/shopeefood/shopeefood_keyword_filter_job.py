...
INPUT_FILE = platform_processed_dir("shopeefood") / "shopeefood_grouped_parsed.json"
OUTPUT_FILE = platform_processed_dir("shopeefood") / "shopeefood_keyword_mentions.json"
SOURCE_NAME = "shopeefood_keyword_filter_job"
...
def main() -> int:
    ...
    if not isinstance(payload, list):
        raise RuntimeError("shopeefood_grouped_parsed.json must be a JSON array")
    ...
    print(f"saved {len(records)} shopeefood records to {OUTPUT_FILE.resolve()}")
    return 0

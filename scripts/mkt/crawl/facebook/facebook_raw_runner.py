...
def flush_brand_pipeline(brand: str, *, do_import: bool) -> bool:
    ...
    steps: list[tuple[str, list[str]]] = [
        (
            "filter",
            [sys.executable, str(PROJECT_ROOT / "scripts/mkt/crawl/facebook/facebook_keyword_filter_job.py")],
        ),
        (
            "format",
            [sys.executable, str(PROJECT_ROOT / "scripts/mkt/crawl/facebook/facebook_format_job.py")],
        ),
    ]
    if do_import:
        steps.append(
            (
                ...
                [
                    sys.executable,
                    str(PROJECT_ROOT / "scripts/source_b/import_keyword_mentions.py"),
                    "--film",
                    "galaxy_cinema",
                ],
            )
...
def crawl_post(driver: webdriver.Chrome, post_url: str) -> dict:
    ...
    album_photo = "photo/?fbid=" in (canonical_url or "").lower() or "photo.php?" in (
        canonical_url or ""
    ).lower()

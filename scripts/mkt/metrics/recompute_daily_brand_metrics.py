...
Sentiment columns (positive_count / negative_count / neutral_count):
  Customer / public buzz only — COALESCE(media_type,'earned') = 'earned'.
  Owned/Paid rows keep sentiment labels on mentions; they are counted in
  owned_count / paid_count / buzz_count but NOT in sentiment_* columns.

Volume columns (buzz_count, post_count, comment_count, owned/paid/earned):
  All media types (total content volume / media mix).
...
# SQL fragment mirrored by is_customer_buzz_media / customer_sentiment_bucket
CUSTOMER_BUZZ_MEDIA_SQL = "COALESCE(media_type, 'earned') = 'earned'"


def is_customer_buzz_media(media_type: Optional[str]) -> bool:
    """True when mention should count toward Customer Sentiment buzz metrics."""
    normalized = (media_type or "earned").strip().lower() or "earned"
    return normalized == "earned"


def customer_sentiment_bucket(
    media_type: Optional[str], sentiment: Optional[str]
) -> Optional[str]:
    """Return positive|negative|neutral for customer buzz, else None.

    Does not rewrite the mention's sentiment label — only metric eligibility.
    """
    if not is_customer_buzz_media(media_type):
        return None
...
def aggregate_customer_sentiment(
    rows: Iterable[dict],
) -> dict[str, int]:
    """Aggregate Customer Pos/Neg/Neu counts from mention-like dicts.

    Each row: {"media_type": ..., "sentiment": ...}
    """
    out = {"positive": 0, "negative": 0, "neutral": 0}
    for row in rows:
        bucket = customer_sentiment_bucket(row.get("media_type"), row.get("sentiment"))
        if bucket:
            out[bucket] += 1
    return out
...
agg_platform AS (
    SELECT
        ...
        COUNT(*) FILTER (
            WHERE sentiment = 'positive' AND COALESCE(media_type, 'earned') = 'earned'
        ) AS positive_count,
        COUNT(*) FILTER (
            WHERE sentiment = 'negative' AND COALESCE(media_type, 'earned') = 'earned'
        ) AS negative_count,
        COUNT(*) FILTER (
            WHERE sentiment = 'neutral' AND COALESCE(media_type, 'earned') = 'earned'
        ) AS neutral_count,

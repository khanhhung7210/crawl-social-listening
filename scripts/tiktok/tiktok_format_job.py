...
def parse_video_record(item: dict) -> dict:
    ...
    return {
        ...
        "post_created_at": parse_created_at(item_struct.get("createTime")).isoformat(),
...
def normalize_created_at(value: object) -> str:
    if isinstance(value, str) and value.strip():
        try:
            ...
            pass
    return parse_created_at(value).isoformat()
...
def resolve_comment_created_at(value: object, label: str, crawled_at: str) -> str:
    ...
    crawl_dt = parse_iso_datetime(crawled_at)
    if crawl_dt is None:

...
OUTPUT_FILE = platform_raw_dir("threads") / "threads_search_results_filtered.json"
...
def main() -> int:
    ...
    if not isinstance(payload, list):
        raise RuntimeError("threads_search_results.json must be a JSON array")
    ...
    print(f"saved {len(filtered)} filtered thread urls to {OUTPUT_FILE.resolve()}")
    return 0

...
THREADS_SEARCH_URL = "https://www.threads.net/search?q={query}"
...
def main() -> int:
    ...
    try:
        ...
        for index, keyword in enumerate(search_terms, start=1):
            ...
            try:
                search_result = search_threads_for_keyword(driver, keyword)
        ...
        print(f"saved {len(results)} thread urls to {OUTPUT_FILE.resolve()}")
        return 0
    finally:
        driver.quit()
...
def search_threads_for_keyword(driver: webdriver.Chrome, keyword: str) -> dict:
    driver.get(THREADS_SEARCH_URL.format(query=quote(keyword)))
    time.sleep(5)
    ...
    for _ in range(MAX_SCROLL_ROUNDS):
        ...
        if empty_rounds >= MAX_EMPTY_ROUNDS_BEFORE_SKIP:
            print(f"[threads-search] no results for keyword={keyword}, skipping")
            return {
                "urls": urls,
                "status": "partial" if urls else "no_results",
                "reason": "empty_limit",

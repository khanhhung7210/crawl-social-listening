...
class DashboardRepository(Protocol):
    ...
    def save_source_config(self, sources: list[dict]) -> dict:
        ...
...
class JsonDashboardRepository:
    ...
    def get_source_config(self) -> dict:
        return {"completed": True, "sources": []}

    def save_source_config(self, sources: list[dict]) -> dict:
        return {"completed": True, "sources": sources}
...
class MongoDashboardRepository:
    ...
    def get_source_config(self) -> dict:
        return {"completed": False, "sources": []}

    def save_source_config(self, sources: list[dict]) -> dict:
        return {"completed": bool(sources), "sources": sources}
...
class PostgresDashboardRepository:
    ...
    def get_source_config(self) -> dict:
        ...
        return {"completed": bool(rows), "sources": rows}
    ...
    def save_source_config(self, sources: list[dict]) -> dict:
        ...
        normalized_sources = [row for row in (self._normalize_source_config(source) for source in sources) if row]
    ...
    def _reputation_modules(self, branch_intelligence: list[dict], daily_branch_metrics: list[dict], evidence_cards: list[dict]) -> list[dict]:
        ...
        # NEW: 4 modules from mockup v16
        modules.extend([
            self._build_crisis_spike_module_v2(),
